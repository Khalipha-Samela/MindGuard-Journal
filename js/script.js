...
function saveJournalEntry(text) {
    try {
        ...
        const previousEntries = entries.map(e => e.text).filter(Boolean);
        ...
        const newEntry = {
            ...
            analysis: generateEnhancedAnalysis(text, previousEntries)
        };
...
function analyzeJournal() {
    ...
    const savedEntry = saveJournalEntry(text);

    if (!savedEntry || !savedEntry.analysis) {
        showToast({
    ...
    setTimeout(() => {
        ...
        updateAnalysisDisplay(savedEntry.analysis);
                
        // Show success message using toast
        showToast({ 
            title: "Analysis Complete", 
            description: "Your journal has been analyzed with enhanced insights.", 
            type: "success" 
        });
...
function signOut() {
    ...
    setTimeout(() => {
        ...
        // localStorage.removeItem('userSession'); // Uncomment if you add session management
...
function saveToHistory() {
    ...
    // Optional: Navigate to history page after a delay
    // setTimeout(() => {
    //     navigateToHistory();
    // }, 1500);
}
